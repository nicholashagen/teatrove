...
public class CoreTest extends AbstractTemplateTest {
    ...
    public CoreTest(String message, TestCase testCase, TestRun testRun) {
    ...
    public static List<Object[]> data() {
        int count = TEST_CASES.length * (TEST_DATA.length * 2);
        List<Object[]> data = new ArrayList<Object[]>(count);
        for (int i = 0; i < TEST_CASES.length; i++) {
            TestCase testCase = TEST_CASES[i];
            for (int j = 0; j < TEST_DATA.length; j++) {
                for (int k = 0; k < TEST_DATA.length; k++) {
                    TestData a = TEST_DATA[j], b = TEST_DATA[k];
                    TestRun testRun = new TestRun(a, b);
                    ...
                    data.add(new Object[] { message, testCase, testRun });
                }
            }
        }
    ...
    public void testScript() throws Throwable {
        runTest(message, testCase, testRun);
    }
